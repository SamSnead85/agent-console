...
The two POST paths require `X-Agent-Console-Interop: 1`, reject browser
`Origin` headers, and accept at most 256 KB per request. All paths are bound to
...
`Content-Type: text/plain` and `X-Agent-Console-Interop: 1`. Each endpoint
...
| Kong AI Gateway | Same metric with `token_type="cache_read_input_tokens"`, `cache_creation_input_tokens`, `cache_write_5m_tokens`, or `cache_write_1h_tokens` | Cache detail when available |
...
window. Their cache detail is a subset of input, so the displayed total is
input plus output. They remain separate from Claude Code telemetry and
...
checked 23 September 2026. Gateway enforcement and budgets are outside this
local adapter.
