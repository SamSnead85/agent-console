...
export function accountingReport({ store, registry, from, to, prices = null }) {
  ...
  // The report is by session hash (the conformance suite's unit, §3.6): a
  // session is looked up as the machine that last reported it holds it.
  const byHash = { get: (hash) => store.sessionByHash?.(hash) ?? store.sessions.get(hash), has: (hash) => Boolean(byHash.get(hash)) };
  const trees = new Map();
  for (const [hash, totals] of sessions) addTotals(at(trees, rootOf(byHash, hash)), totals);
  const sessionFacts = {};
  for (const hash of [...sessions.keys()].sort()) {
    const facts = byHash.get(hash);
    sessionFacts[hash] = {
      tool: facts?.tool ?? null,
      parentSessionHash: facts?.parentSessionHash ?? null,
      root: rootOf(byHash, hash),
