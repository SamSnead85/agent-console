...
import { PERIODS as CONSOLE_PERIODS, periodRange, priceStatus, STACK_STEPS, stackOf } from "./aggregate.js";
...
export async function projectsPayload({ store, registry, names, period, demo, git = null, now = Date.now() }) {
  ...
  const add = (projectHash, bucket, sessionHash) => {
    ...
    if (!p) { p = { name: nameOf(projectHash), projectHash, tokens: 0, cost: emptyCost(), sessions: new Set(), sessionsKnown: true, branches: new Set() }; byProject.set(projectHash, p); }
  ...
  const topOf = (sessionHash) => {
    const session = store.sessions.get(sessionHash);
    return (session && session.isSubagent && store.sessions.get(session.parentSessionHash)) || session;
  };
  ...
  store.eachBucket(scanFrom, minuteNow + MINUTE, (minute, bucket) => {
    ...
    const top = mine ? topOf(bucket.sessionHash) : null;
    ...
    if (range.basis === "minutes" && top && minute >= from && minute < range.to) add(top.projectHash, bucket, top.sessionHash);
  });
