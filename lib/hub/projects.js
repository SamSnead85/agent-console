...
export async function projectsPayload({ store, registry, names, period, demo, git = null, now = Date.now() }) {
  ...
  store.eachBucket(now - span, now + 60_000, (_minute, bucket) => {
  ...
  if (demo) {
    for (const p of ranked) {
      ...
      if (g) repoOf.set(p.name, { name: p.name, commits: Math.round(g.commits * scale), added: Math.round(g.added * scale), removed: Math.round(g.removed * scale), prsMerged: Math.round(g.prsMerged * scale) });
    }
    ...
    for (const p of ranked.slice(0, MAX_GIT_PROJECTS)) {
      ...
      if (!dir) continue;
      try {
        const stats = await gitStatsForPeriod(git, [dir], now - span);
        const repo = stats.repos[0];
        if (repo) repoOf.set(p.name, { name: repo.name, commits: repo.commits, added: repo.added, removed: repo.removed, prsMerged: repo.prsMerged });
