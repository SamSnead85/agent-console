...
export async function projectsPayload({ store, registry, names, period, demo, git = null, now = Date.now() }) {
  ...
  const topOf = (sessionHash) => (sessionHash ? store.sessions.get(rootOf(sessionHash)) || store.sessions.get(sessionHash) : null);
  ...
  store.eachBucket(scanFrom, minuteNow + MINUTE, (minute, bucket) => {
    ...
    if (range.basis === "minutes" && top && minute >= from && minute < range.to) add(top.projectHash, bucket, top.sessionHash, bucket.sessionHash);
  });
