...
export async function projectsPayload({ store, registry, names, period, demo, git = null, now = Date.now() }) {
  ...
  if (demo) {
    for (const p of ranked) {
      ...
      if (g) repoOf.set(p.name, { name: p.name, commits: Math.round(g.commits * scale), added: Math.round(g.added * scale), removed: Math.round(g.removed * scale), prsMerged: Math.round(g.prsMerged * scale), defaultMerges: Math.round(g.prsMerged * scale) });
    }
