 ...
 */
export async function choosePort({ port, host, explicit = false, demo = false, tries = 20, fetchImpl, avoid = [] }) {
  ...
  for (let next = port + 1; next <= Math.min(65_535, port + tries); next += 1) {
    if (avoid.includes(next) || await agentConsoleAt(next, fetchImpl)) continue;
