 ...
 * exactly: each class's tokens times that class's rate. A bucket with any
 * unpriced record cannot be split and returns null; its dollars stay whole.
...
export function buildConsole({ store, registry, names = null, now, hub }) {
  ...
  store.eachBucket(weekStart, minuteNow + MINUTE, (minute, bucket) => {
    ...
    if (!s) {
      s = { day: 0, hour: 0, fiveMin: 0, firstAt: minute, lastAt: minute, spark: new Array(SPARK_BARS).fill(0),
        classes: { fresh: 0, output: 0, cacheWrite: 0, cacheRead: 0 }, usd: 0, pricedN: 0, unpricedN: 0 };
      sessionAgg.set(bucket.sessionHash, s);
    }
    s.day += tokens;
    for (const k of CLASSES) s.classes[k] += bucket[k];
  ...
  for (const session of store.sessions.values()) {
    ...
    if (s) {
      ...
      for (const k of CLASSES) lane.classes[k] += s.classes[k];
