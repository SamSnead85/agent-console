 ...
 *
 * The rules this module keeps, because each is a way the screen could lie:
 *
...
export function deviceStatus(device, now) {
  ...
  return now - device.lastContactAt <= limit ? "reporting" : "silent";
}
...
function emptyAgg() {
  return {
    ...
    n: 0, usd: 0, pricedN: 0, unpricedN: 0, unpricedTokens: 0,
...
function addBucket(agg, bucket, tokens) {
  ...
  if (!m) { m = { tokens: 0, usd: 0, n: 0, unpricedN: 0 }; agg.models.set(bucket.model, m); }
  m.tokens += tokens; m.usd += bucket.usd; m.n += bucket.n; m.unpricedN += bucket.unpricedN;
  if (bucket.unpricedN) agg.unpricedModels.add(bucket.model);
...
export function buildConsole({ store, registry, names = null, now, hub }) {
  ...
  store.eachBucket(Math.min(weekStart, dayFrom), now + DAY, (minute, bucket) => {
    ...
    if (minute >= burnFrom) {
      s.fiveMin += tokens;
      if (reporting.has(bucket.deviceId)) { burnTokens += tokens; burnUsd += bucket.usd; }
    }
  });
  ...
  for (const lane of lanes.values()) {
    ...
    const state = unavailable ? (device.status === "revoked" ? "revoked" : "silent")
    ...
    laneRows.push({
      ...
      device: { id: device.id, label: device.label, person: device.person || null, local: Boolean(device.local), status: device.status, lastContactAt: device.lastContactAt || null },
      spark: lane.spark,
  ...
  const rank = { live: 0, idle: 1, silent: 2, revoked: 3 };
  ...
  for (const d of devices) {
    ...
    for (const [src, dst] of [[dayByDevice, p.day], [weekByDevice, p.week]]) {
      ...
      if (!a) continue;
      for (const k of ["total", ...CLASSES, "unknownFresh", "unknownOutput", "unknownCacheWrite", "unknownCacheRead", "n", "usd", "pricedN", "unpricedN", "unpricedTokens"]) dst[k] += a[k];
      for (const [model, m] of a.models) {
        let t = dst.models.get(model);
        if (!t) { t = { tokens: 0, usd: 0, n: 0, unpricedN: 0 }; dst.models.set(model, t); }
        t.tokens += m.tokens; t.usd += m.usd; t.n += m.n; t.unpricedN += m.unpricedN;
      }
      for (const m of a.unpricedModels) dst.unpricedModels.add(m);
  ...
  return {
    ...
    burn: {
      ...
      usdPerMinute: burnUsd / elapsed,
      reporting: reporting.size,
      excluded: silent.map((d) => ({ id: d.id, label: d.label, lastContactAt: d.lastContactAt })),
    },
