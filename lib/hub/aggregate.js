...
import { CLASSES, MINUTE, sessionKey } from "./store.js";
import { contextHealth, agentTree } from "../analysis/index.js";
import { priceRecord } from "../collector/pricing.js";
import { ACTIVITY_KINDS, ACTIVITY_WINDOW_MINUTES } from "../collector/activity.js";
import { alertsTodayOf } from "./alert-day.js";
...
function addBucket(agg, bucket, tokens, split = null) {
  ...
  // Sessions per machine: the same hash on two machines is two sessions.
  if (bucket.sessionHash) agg.sessions.add(sessionKey(bucket.deviceId, bucket.sessionHash));
...
/**
 * A session's lane: the top of its parent chain among every session the
 * console keeps, however long ago that parent last reported. The lanes, the
 * Team counts, the alerts and the Projects view all fold a subagent into the
 * same lane with it, so their session counts agree.
 */
export function laneRootResolver(store) {
  const cache = new Map();
  // One machine's session tree: a subagent's parent is looked for on the same
  // machine, so a session hash two machines share never joins their lanes.
  return (deviceId, hash) => {
    const key = sessionKey(deviceId, hash);
    if (cache.has(key)) return cache.get(key);
    let h = hash;
    for (let guard = 0; guard < 32; guard += 1) {
      const row = store.sessions.get(sessionKey(deviceId, h));
      if (!row || !row.parentSessionHash || row.parentSessionHash === h || !store.sessions.has(sessionKey(deviceId, row.parentSessionHash))) break;
...
/**
 * A lane's key: the first 16 characters of its session hash, which is what an
 * alert's `laneHash` is matched by. When two machines have lanes under the
 * same session hash, each key also names its machine, so neither hides the other.
 */
export function laneKeyResolver(store) {
  const machines = new Map();   // sessionHash -> Set(deviceId)
  for (const s of store.sessions.values()) {
    let set = machines.get(s.sessionHash);
    if (!set) { set = new Set(); machines.set(s.sessionHash, set); }
    set.add(s.deviceId);
  }
  return (deviceId, hash) => (machines.get(hash)?.size > 1
    ? `${hash.slice(0, 16)}-${createHash("sha256").update(String(deviceId)).digest("hex").slice(0, 8)}`
    : hash.slice(0, 16));
}
 ...
 * `alertDay`: the day's alert counter's reading (lib/hub/alert-day.js), or
 * null when the embedding keeps none.
 */
export function buildConsole({ store, registry, names = null, now, hub, alerts = [], signals = null, alertDay = null }) {
  ...
  const laneRootOf = laneRootResolver(store);
  ...
  const tallyPeriod = (key, bucket, tokens) => {
    ...
    const root = bucket.sessionHash ? sessionKey(bucket.deviceId, laneRootOf(bucket.deviceId, bucket.sessionHash)) : null;
  ...
  // An alert's machine: named on it, or, from an older source, the machine that last reported its session.
  const alertMachine = (a) => a.deviceId ?? store.sessionByHash?.(a.sessionHash)?.deviceId ?? null;
  const alertSessions = new Set(alerts.filter((a) => a && a.kind !== "loop").map((a) => sessionKey(alertMachine(a) ?? "", a.sessionHash)));
  ...
  const sessionAgg = new Map();   // sessionKey(deviceId, sessionHash) -> { day, hour, spark[], fiveMin }
  ...
  store.eachBucket(weekStart, minuteNow + MINUTE, (minute, bucket) => {
    ...
    const bucketSession = sessionKey(bucket.deviceId, bucket.sessionHash);
    if (alertSessions.has(bucketSession)) {
      let m = alertMinutes.get(bucketSession);
      if (!m) { m = new Map(); alertMinutes.set(bucketSession, m); }
      m.set(minute, (m.get(minute) || 0) + tokens);
    }
    if (minute < dayFrom) return;
    let s = sessionAgg.get(bucketSession);
    if (!s) {
      ...
      sessionAgg.set(bucketSession, s);
    }
    s.day += tokens;
  ...
  // Each machine's sessions make their own trees: a session hash two machines
  // share is two lanes, each with its own machine, tokens and activity.
  const keyOf = (session) => sessionKey(session.deviceId, session.sessionHash);
  const inWindow = [...store.sessions.values()].filter((s) => s.lastAt >= dayFrom - MINUTE);
  const treeInput = new Map(inWindow.map((s) => [keyOf(s), s]));
  for (const s of inWindow) {
    let parent = s.parentSessionHash;
    for (let guard = 0; parent && !treeInput.has(sessionKey(s.deviceId, parent)) && guard < 32; guard += 1) {
      const row = store.sessions.get(sessionKey(s.deviceId, parent));
      if (!row) break;
      treeInput.set(keyOf(row), row);
      parent = row.parentSessionHash;
    }
  }
  const byMachine = new Map();
  for (const session of treeInput.values()) {
    if (!byMachine.has(session.deviceId)) byMachine.set(session.deviceId, []);
    byMachine.get(session.deviceId).push(session);
  }
  const rootBySession = new Map();   // sessionKey -> root sessionKey
  const treeByRoot = new Map();      // root sessionKey -> agent rows
  for (const [deviceId, list] of byMachine) {
    const rows = agentTree(list.map((session) => ({
      sessionHash: session.sessionHash, parentSessionHash: session.parentSessionHash,
      model: session.model, firstAt: sessionAgg.get(keyOf(session))?.firstAt,
      lastAt: sessionAgg.get(keyOf(session))?.lastAt,
      tokens: sessionAgg.get(keyOf(session))?.day ?? null,
      outcome: 'unknown',
    })));
    for (const row of rows) {
      const root = sessionKey(deviceId, row.rootSessionHash);
      rootBySession.set(sessionKey(deviceId, row.sessionHash), root);
      if (!treeByRoot.has(root)) treeByRoot.set(root, []);
      treeByRoot.get(root).push(row);
    }
  }
  const lanes = new Map();
  for (const session of store.sessions.values()) {
    if (session.lastAt < dayFrom - MINUTE) continue;
    const top = store.sessions.get(rootBySession.get(keyOf(session))) || session;
    let lane = lanes.get(keyOf(top));
    if (!lane) {
      lane = { top, children: [], lastAt: 0, day: 0, hour: 0, fiveMin: 0, spark: new Array(SPARK_BARS).fill(0),
        classes: zeroClasses(), unknown: zeroClasses(), usd: 0, pricedN: 0, unpricedN: 0 };
      lanes.set(keyOf(top), lane);
    }
    if (session !== top) lane.children.push(session);
    const s = sessionAgg.get(keyOf(session));
    if (s) {
  ...
  const laneKeyOf = laneKeyResolver(store);
  // The activity window's first minute: coverage is judged over the whole window.
  const activityFrom = minuteNow - (ACTIVITY_WINDOW_MINUTES - 1) * MINUTE;
  const unheard = { state: "unknown", since: null, reason: "not-heard" };
  const activityCoverageOf = (device) => (signals?.activityCoverage ? signals.activityCoverage(device, activityFrom)
    : signals?.activityShared?.(device) ? { state: "complete", since: null, reason: null } : unheard);
  function laneActivity(lane, device) {
    const coverage = activityCoverageOf(device);
    const shared = coverage.state === "complete" || coverage.state === "partial";
    const snap = shared ? signals.activity([lane.top.sessionHash, ...lane.children.map((c) => c.sessionHash)], device) : null;
    const counted = Boolean(snap) && (ACTIVITY_KINDS.some((k) => snap.calls[k] > 0) || snap.results.ok > 0 || snap.results.error > 0);
    // Covered for the whole window, nothing held is no tool call: zero, known.
    // Covered only for part of it, what is held is a floor and nothing held is
    // unavailable (null), never zero; so is everything a machine does not share.
    const activity = counted ? { window: snap.window, calls: snap.calls, results: snap.results }
      : coverage.state === "complete" ? { window: `${ACTIVITY_WINDOW_MINUTES}m`, calls: Object.fromEntries(ACTIVITY_KINDS.map((k) => [k, 0])), results: { ok: 0, error: 0 } }
        : null;
    return { activity, lastTool: snap?.lastTool ?? null, activityShared: shared, activityCoverage: coverage };
  }
  for (const lane of lanes.values()) {
    ...
    laneRows.push({
      key: laneKeyOf(lane.top.deviceId, lane.top.sessionHash),
      state,
      project,
      ...
      agentTree: (treeByRoot.get(keyOf(lane.top)) || []).map((agent) => ({ ...agent, modelLabel: modelLabel(agent.model),
  ...
  // What each machine shares beyond its records, over the windows they are
  // shown for (alerts: the last hour; activity: the last five minutes), and
  // how many of its entries were refused for being dated in the future.
  const sharingOf = (d) => ({
    alerts: signals?.alertsCoverage ? signals.alertsCoverage(d)
      : signals?.alertsWatched?.(d) ? { state: "complete", since: null, reason: null } : unheard,
    activity: activityCoverageOf(d),
    rejectedFuture: signals?.rejectedFuture ? signals.rejectedFuture(d) : 0,
  });
  const sharingById = new Map(devices.map((d) => [d.id, sharingOf(d)]));
  ...
  // Each day says whether the rollup holds it whole: from `windows["30d"].since`
  // on (a first read fills the month; lib/hub/store.js). A day before it,
  // such as the edge of the first start's retention, is drawn as partial.
  const monthSince = store.dailySince ?? null;
  series["30d"] = { start: month.from, step: DAY, values: monthValues, classes: monthClasses, timeZone: "UTC",
    whole: monthValues.map((_, i) => monthSince !== null && new Date(month.from + i * DAY).toISOString().slice(0, 10) >= monthSince) };
  ...
  for (const row of laneRows) {
    ...
    if (meta.local) {
      // sessions, live and subagents per project on this machine, over every lane — the Projects view's own counts, never a sum over the rows drawn
      const p = laneTotals.byLocalProject[meta.projectHash] ??= { sessions: 0, live: 0, subagents: 0, hourSpark: new Array(SPARK_BARS).fill(0) };
      p.sessions += 1; if (row.state === "live") p.live += 1; p.subagents += meta.children;
  ...
  // a month's caption.
  // Every current machine, its person, and every tool seen in any period
  // has a row in every period: a minute period with nothing in it is a
  // counted 0 (the minute detail is kept), never an absent row a screen
  // could mistake for "not kept". Only the daily rollup's sessions are null.
  const everyDevice = devices.filter((d) => d.status !== "revoked").map((d) => d.id);
  const everyPerson = [...new Set(everyDevice.map(personOf))];
  const everyTool = [...new Set([...Object.keys(laneTotals.byTool), ...Object.values(periodTally).flatMap((t) => [...t.byTool.keys()])])];
  const tallyOut = (map, kept, ids) => {
    const out = Object.fromEntries(ids.map((id) => [id, { sessions: kept ? 0 : null, tokens: 0 }]));
    for (const [id, e] of map) out[id] = { sessions: kept ? e.lanes.size : null, tokens: e.tokens };
    return out;
  };
  laneTotals.periods = Object.fromEntries(Object.keys(PERIODS).map((key) => {
    ...
    return [key, { basis: ranges[key].basis, sessionsKept: kept,
      ...
      byTool: tallyOut(t.byTool, kept, everyTool), byDevice: tallyOut(t.byDevice, kept, everyDevice), byPerson: tallyOut(t.byPerson, kept, everyPerson) }];
  }));
  ...
  const alertRows = alerts.filter(Boolean).map((a) => {
    // An alert names its machine: its lane is that machine's session tree.
    const machine = alertMachine(a);
    const root = machine ? laneRootOf(machine, a.sessionHash) : a.sessionHash;
    const top = (machine ? store.sessions.get(sessionKey(machine, root)) || store.sessions.get(sessionKey(machine, a.sessionHash)) : null) || null;
    const deviceId = machine ?? top?.deviceId ?? null;
    ...
    const lane = top ? { key: laneKeyOf(top.deviceId, root), projectHash, deviceId, ...(displayName ? { displayName } : {}) } : null;
    ...
    if (a.kind === "spike" || a.kind === "stall") Object.assign(out, magnitude(alertMinutes.get(sessionKey(machine ?? "", a.sessionHash)), a.at));
    return out;
  }).sort((x, y) => y.at - x.at);
  // Watched: a machine whose alerts this console holds for the hour, or for
  // part of it (`since`, `reason`: before then, "no alert" is not known).
  const alertCoverage = (d) => sharingById.get(d.id)?.alerts ?? unheard;
  const isWatched = (d) => ["complete", "partial"].includes(alertCoverage(d).state);
  const watched = current.filter(isWatched);
  const partly = watched.filter((d) => alertCoverage(d).state === "partial").sort((a, b) => alertCoverage(b).since - alertCoverage(a).since);
  const alertsCoverage = { watched: watched.length, unwatched: current.length - watched.length,
    unwatchedDevices: current.filter((d) => !isWatched(d)).map((d) => d.id),
    since: partly.length ? alertCoverage(partly[0]).since : null, reason: partly.length ? alertCoverage(partly[0]).reason : null,
    byDevice: Object.fromEntries(current.map((d) => [d.id, alertCoverage(d)])) };
  ...
  return {
    ...
    team: { perMachine },
    alerts: alertRows,
    // The day's count, exact: never the length of the list above, which is bounded.
    alertsToday: alertsTodayOf(alertDay, alertRows, now),
    alertsCoverage,
