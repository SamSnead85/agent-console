...
export function buildConsole({ store, registry, names = null, now, hub, alerts = [], signals = null }) {
  ...
  // on. A first read follows configured retention; days before the earliest
  // complete daily rollup are drawn as partial.
  ...
  return {
    ...
    team: { perMachine },
    alerts: alertRows,
    // Counts describe the bounded list, never a complete daily ledger.
    alertsToday: alertsTodayOf(alertRows, now),
    alertsCoverage,
