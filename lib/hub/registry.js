...
export function createRegistry({ dir = null, now = () => Date.now() } = {}) {
  ...
  return {
    ...
    redeem(rawCode, { name, previousToken } = {}) {
      ...
      const proven = typeof previousToken === "string" && TOKEN_PATTERN.test(previousToken)
        ? [...devices.values()].find((d) => !d.local && (d.tokenHash === sha256(previousToken) || d.retiredTokenHash === sha256(previousToken)))
        : null;
      // `leave` deletes the machine's enrolment, token included. A link for the
      // same person and the same machine name brings back the entry that left
      // (not one the console removed), with its history, rather than adding a
      // second machine of that name.
      const wantedName = (cleanLabel(asked, null) || invitation.machine || "").toLowerCase();
      const departed = proven || !wantedName ? null : [...devices.values()].find((d) => !d.local && d.leftAt && d.revokedAt === d.leftAt
        && (d.person || "").toLowerCase() === (invitation.person || "").toLowerCase() && d.label.toLowerCase() === wantedName);
      const previous = proven || departed;
      let device;
      if (previous) {
        device = previous;
    ...
    touch(id, { freshness, backlog, coverage, at = now() } = {}) {
      const device = devices.get(id);
      if (!device) return;
      device.lastContactAt = at;
      // What the machine's collector could not count, by reason (counts only).
      if (coverage) { device.coverage = { ...coverage }; if (!Number.isFinite(device.coverageSince)) device.coverageSince = at; }
