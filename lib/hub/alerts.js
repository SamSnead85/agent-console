...
export function createAlerts({ repeat = 5, spikeFactor = 3, stallMinutes = 5,
  ...
  return {
    ...
    /**
     * What a reporter sends in its next envelope: kind, minute, salted
     * hashes and a count — nothing else (docs/COLLECTOR-CONTRACT.md).
     * `take()` hands over the alerts not yet acknowledged; `ack()` drops them
     * once the console has taken the envelope that carried them.
     */
    outbox() {
      let taken = [];
      return {
        take() {
          const t = now();
          taken = unsent.filter((a) => a.at >= t - ALERT_HISTORY_MS);
          return taken.map((a) => ({ id: a.id, kind: a.kind, at: minuteIso(a.at), sessionHash: a.sessionHash,
            count: Number.isSafeInteger(a.count) && a.count >= 0 ? a.count : 0, historical: historical(a, t) }));
        },
        ack() { const sent = new Set(taken); unsent = unsent.filter((a) => !sent.has(a)); taken = []; },
      };
    },
