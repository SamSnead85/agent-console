...
export function createAlerts({ repeat = 5, spikeFactor = 3, stallMinutes = 5,
  notify = false, now = () => Date.now(), live = () => true, onAlert = () => {}, day = null } = {}) {
  ...
  const stateFor = (id) => {
    ...
    if (!state) {
      state = { analysis: emptyAlertState(), observedTokens: 0, seenCalls: new Set(), lastAt: 0 };
      sessions.set(id, state);
    }
    return state;
  };
  // MEMORY. A first read replays up to a month of lines; an alert older than
  // the day kept is never shown, so while replaying history (`live()` false)
  // a line older than that is not analysed, and a session quiet for longer
  // than the day is let go (its call identities alone can run to thousands).
  const STALE_MS = ALERT_HISTORY_MS + 60 * MINUTE;
  let lines = 0;
  const replaying = () => { try { return live() !== true; } catch { return true; } };
  const pruneQuiet = (t) => { for (const [id, s] of sessions) if (s.lastAt && s.lastAt < t - STALE_MS) sessions.delete(id); };
  ...
  const observeLine = ({ tool, line, records = [], sessionHash, parentSessionHash, projectHash,
    ...
    if (sourceAt !== null && sourceAt < now() - STALE_MS && replaying()) return;
    if (++lines % 5_000 === 0) pruneQuiet(now());
    const state = stateFor(sessionHash);
    if (sourceAt !== null && sourceAt > state.lastAt) state.lastAt = sourceAt;
    ...
    const observe = (kind, fields = {}) => {
      ...
      for (const signal of next.signals) {
        ...
        // Counted for the day however many the list above still keeps (lib/hub/alert-day.js).
        day?.add(shown, eventAt);
        onAlert(shown);
