...
export function createFleetSignals({ now = () => Date.now(), startedAt = now(), day = null, firstStart = false } = {}) {
    ...
    accept(deviceId, { share, alerts, activity: entries, lost } = {}, seenAt = now()) {
      ...
      if (Array.isArray(alerts)) {
        for (const a of alerts) {
          ...
          // The day's count holds every accepted alert, whatever this list keeps (lib/hub/alert-day.js).
          day?.add({ kind: a.kind, at, historical: a.historical === true }, seenAt);
        }
