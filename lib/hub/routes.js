...
import { validateRecords, freshnessFor, backlogFor } from "../collector/transport.js";
...
export function createHubRoutes({ config, registry, store, names, local, version, root }) {
  ...
  // Per device. A reporter sends one batch of up to 500 records at a time, so
  // 600 a minute lets a first sync of 300,000 records through in a minute; past
  // that the 429 names its wait (Retry-After) and the reporter honours it,
  // keeping every batch already acknowledged.
  ...
  return async function handle(req, res, url, { sendJson, readBody, send, port }) {
    ...
    if (url === "/api/ingest" && req.method === "POST") {
      ...
      try {
        ...
        const backlog = backlogFor(envelope.backlog);
