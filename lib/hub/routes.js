...
import { joinCommand, releasePage, releaseUrl } from "../invocation.js";
...
export function createReportingHandler({ config, registry, store, version, publicDir, onChange = () => {}, onEvent = () => {} }) {
...
export function createConsoleHandler({ config, registry, store, names, local, admin, version, publicDir, reporting, git = null, alerts = null, interop = null,
  onSignInLink = null, networkCommand = null }) {
  let cached = null;
  ...
  function consolePayload() {
    ...
    value.alerts = alerts?.list() || [];
    value.interop = config.demo ? { otel: { available: true, tokens: { input: 23_000, output: 8_000, cacheRead: 41_000, cacheWrite: 2_000, total: 74_000 }, receivedAt: now },
      kong: { available: true, tokens: { input: 32_000, output: 10_000, cacheRead: 6_000, cacheWrite: 1_000, total: 42_000 }, receivedAt: now },
      litellm: { available: true, tokens: { input: 30_000, output: 8_000, cacheRead: 5_000, cacheWrite: 500, total: 38_000 }, receivedAt: now } }
      : interop?.snapshot(now) || null;
    cached = { at: now, value };
    return value;
  }
  ...
  return {
    ...
    async handle(req, res) {
      ...
      if (config.interop && interopPath) {
        // TODO: scrape token pending. /metrics and the telemetry ingest must
        // authenticate with a dedicated scrape token derived from the console
        // key (HMAC), never with the console key itself, which is never sent
        // over the wire. Until that token exists they refuse every request.
        res.writeHead(401, headers('text/plain; charset=utf-8', { 'www-authenticate': 'Bearer realm="agent-console-metrics"' }));
        res.end('scrape token pending: /metrics and telemetry ingest are not available yet\n');
        return;
      }
      ...
      if (config.interop && url === '/metrics' && req.method === 'GET') {
        ...
        res.end(formatInteropMetrics(view.day.tokens, view.interop || interop.snapshot()));
        return;
      }
