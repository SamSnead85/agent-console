...
import { validateRecords, freshnessFor, backlogFor, coverageFor, alertsFor, activityFor, isPrivateHost } from "../collector/transport.js";
  ...
  return async function handle(req, res, { secure }) {
    ...
    if (url === "/api/ingest") {
      ...
      try {
        ...
        // Opt-in alerts and tool activity: counts, kinds, minutes and salted hashes only.
        extras = { alerts: alertsFor(envelope.alerts), activity: activityFor(envelope.activity) };
        for (const e of extras.activity || []) if (Date.parse(e.at) > Date.now() + 86_400_000) throw new Error("future");
      } catch {
...
/** Where each machine's activity and alert watch come from: this machine's own reading, or its reporter's opt-in. */
export function consoleSignals({ alerts = null, fleet = null, activity = null }, now) {
  const book = (device) => (device.local ? activity : fleet?.sharesActivity(device.id) ? fleet.activity : null);
  return {
    activityShared: (device) => Boolean(book(device)),
    activity: (hashes, device) => book(device)?.snapshot(hashes, now) ?? null,
    results: (hash, device) => book(device)?.results(hash) ?? null,
    alertsWatched: (device) => (device.local ? Boolean(alerts) : Boolean(fleet?.watchesAlerts(device.id))),
  };
}
