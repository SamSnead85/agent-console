...
import { validateRecords, freshnessFor, backlogFor, backfillFor, coverageFor, extrasFor, isPrivateHost } from "../collector/transport.js";
...
/*
 * How likely another machine can reach an interface, lowest first: wired and
 * wireless adapters (macOS, Linux and Windows names), then VPN and overlay
 * networks, then anything unknown, then virtual switches and bridges that
 * only this machine's guests can reach (Hyper-V and WSL's vEthernet, Docker,
 * VirtualBox, VMware, libvirt, Wi-Fi Direct).
 */
const PHYSICAL = /^(en|eth|wl|wlan|wlp|enp|eno|ens|Ethernet|Wi-?Fi|WLAN|Local Area Connection(?!\*))/iu;
const OVERLAY = /^(utun|tailscale|wg|tun|ppp|ipsec|zt)/iu;
const GUESTS = /(vEthernet|WSL|Default Switch|docker|^br-|^veth|virbr|vmnet|vboxnet|VirtualBox|VMware|Hyper-V|Bluetooth|isatap|Teredo|^lxd|^lxcbr|^cni|^flannel|^podman|Loopback|Local Area Connection\*)/iu;
const PRIVATE_V4 = (a) => /^(10\.|192\.168\.|172\.(1[6-9]|2\d|3[01])\.)/u.test(a);

/** True inside Windows Subsystem for Linux, where this machine's own addresses are usually not the LAN's. */
export function inWsl(env = process.env, release = os.release()) {
  return Boolean(env.WSL_DISTRO_NAME || env.WSL_INTEROP) || /microsoft/iu.test(release);
}

/**
 * The addresses another machine could use to reach the reporting port, the
 * likeliest first: the first is the one join links carry. `advertise`
 * (--advertise) is put first as given: what to use when this machine cannot
 * tell (WSL, several adapters, a port forward). `wsl` says the addresses were
 * read inside WSL, where they are usually reachable from this computer only.
 */
export function hubAddresses(listen, port, { advertise = null, interfaces = os.networkInterfaces(), wsl = inWsl() } = {}) {
  const host = (address) => (address.includes(":") ? `[${address}]` : address);
  const loopbackOnly = isLoopbackAddress(listen) || listen === "localhost";
  if (loopbackOnly) return { network: false, urls: [`http://127.0.0.1:${port}`], advertised: false, wsl: false };
  const first = advertise ? [`http://${host(advertise)}:${port}`] : [];
  if (listen !== "0.0.0.0" && listen !== "::") {
    const own = `http://${host(listen)}:${port}`;
    return { network: true, urls: [...first, ...(first.includes(own) ? [] : [own])], advertised: Boolean(advertise), wsl };
  }
  const found = [];
  for (const [name, list] of Object.entries(interfaces || {})) {
    for (const entry of list || []) {
      if (entry.internal || entry.family !== "IPv4" || /^169\.254\./u.test(entry.address)) continue;
      const weight = GUESTS.test(name) ? 3 : PHYSICAL.test(name) ? 0 : OVERLAY.test(name) ? 1 : 2;
      found.push({ weight, lan: PRIVATE_V4(entry.address) ? 0 : 1, url: `http://${entry.address}:${port}` });
    }
  }
  found.sort((a, b) => a.weight - b.weight || a.lan - b.lan);
  const urls = [...first, ...found.map((u) => u.url).filter((u) => !first.includes(u))];
  return { network: true, urls: urls.length ? urls : [`http://127.0.0.1:${port}`], advertised: Boolean(advertise), wsl };
}

/** The version, date and size of the offline price table this console prices with. */
export function priceTableInfo(prices) {
  if (!prices || typeof prices !== "object") return null;
  const rows = Array.isArray(prices.rows) ? prices.rows : [];
  const verified = rows.map((r) => r?.verifiedOn).filter((d) => typeof d === "string" && /^\d{4}-\d{2}-\d{2}$/u.test(d)).sort();
  return { v: Number.isInteger(prices.v) ? prices.v : null, checkedOn: typeof prices.inventoryCheckedOn === "string" ? prices.inventoryCheckedOn : null,
    lastVerifiedOn: verified.at(-1) ?? null, currency: typeof prices.currency === "string" ? prices.currency : null,
    basis: typeof prices.basis === "string" ? prices.basis : null, models: rows.length };
}
...
export function createReportingHandler({ config, registry, store, fleet = null, version, publicDir, onChange = () => {}, onEvent = () => {} }) {
  ...
  return async function handle(req, res, { secure }) {
    ...
    if (url === "/api/ingest") {
      ...
      let freshness, backlog, coverage, extras, backfill;
      try {
        ...
        backfill = backfillFor(envelope.backfill);
      ...
      // A first read's past days go to the daily totals only; the allowance is for minutes kept.
      if (store.quotaLeft(device.id) < (store.quotaNeeded ? store.quotaNeeded(envelope.records) : envelope.records.length)) {
        sendJson(res, 429, { ok: false, reason: "this machine's daily allowance is used up" }, { extra: { "retry-after": String(secondsToUtcMidnight()) } });
        return;
      }
      const receipt = store.ingest(device.id, envelope.records, backfill ? { backfill: backfill.from } : {});
      // The first delivery is complete when its backlog is: its past days are done.
      if (backfill && backlog && backlog.delivered === backlog.total) store.endBackfill?.(device.id);
...
const MINUTE = 60_000;
/** The longest interval a live reporter waits between reports (lib/reporter.js, LIVE_INTERVAL_S). */
 ...
 *   partial     whole only since `since`: what is held is a floor, and a lane with nothing held is unavailable
 ...
 *   console-restarted    kept in memory; this console started inside the window, and holds nothing from before
 *   first-start          as console-restarted, on this console's very first start: there was no earlier run
 *   sharing-started      this console first heard the machine say it shares them at `since`; before, nobody can say
 *   outbox-overflow      the machine's reporter had to drop pending extras from minutes up to just before `since`
 ...
 * Coverage is never dated before the "on" that declared it: a run with
 * sharing off may already have read past what happened earlier.
 */
export function coverageOf(declared, from, startedAt, { firstStart = false } = {}) {
  ...
  let since = Number.isFinite(declared.since) ? declared.since : null;
  let reason = null;
  if (since !== null && since > from) reason = restartExplains(declared, since, from, startedAt) ? (firstStart ? "first-start" : "console-restarted") : "sharing-started";
  // A loss the reporter declared inside the window: whole only after its last minute.
  for (const gap of declared.gaps ?? []) {
    if (gap.to < from) continue;
    const after = gap.to + MINUTE;
    if (reason === null || after > since) { since = after; reason = "outbox-overflow"; }
  }
  return reason ? { state: "partial", since, reason } : { state: "complete", since, reason: null };
}

/**
 * A restart is the reason only when this console started inside the window
 * and the "on" is the first thing it heard from the machine, within one live
 * report interval of its start: a reporter that was sharing all along is
 * heard that soon. An "on" heard later, or after this console had heard the
 * machine say "off" or nothing, means the machine began sharing then.
 */
function restartExplains(declared, since, from, startedAt) {
  if (!Number.isFinite(startedAt) || startedAt <= from) return false;
  if (declared.after && declared.after !== "on") return false;
  return since - startedAt <= RESTART_GRACE_MS;
}
...
export function consoleSignals({ alerts = null, fleet = null, activity = null }, now) {
  ...
  // A console on its first start never "restarted": there was nothing before it.
  const firstStart = Boolean(fleet?.firstStart);
  const declared = (device, kind) => {
    if (!device.local) return fleet?.coverage(device.id, kind) ?? null;
  ...
  const activityCoverage = (device, from) => coverageOf(declared(device, "activity"), from, startedAt, { firstStart });
  const alertsCoverage = (device, from = now - ALERT_WINDOW_MS) => coverageOf(declared(device, "alerts"), from, startedAt, { firstStart });
...
export function createConsoleHandler({ config, registry, store, names, local, admin, version, publicDir, reporting, git = null, alerts = null, alertDay = null, interop = null,
  fleet = null, activity = null, onSignInLink = null, networkCommand = null }) {
  let cached = null;
  let lastSignInPrint = 0;

  function hubInfo() {
    const addresses = hubAddresses(config.listen, reporting.port, { advertise: config.advertise ?? null });
    return {
      ...
      urls: addresses.urls,
      // Read inside WSL: the join link's address is likely reachable from this computer only.
      wsl: addresses.wsl && !addresses.advertised,
      prices: priceTableInfo(store.prices),
      ...
      local: local ? {
        ...
        // Every folder read for transcripts, whether it exists, and how many it holds.
        roots: (local.status.rootsRead || []).filter((r) => r.exists !== false || !r.optional || r.tool === "codex")
          .map((r) => ({ tool: r.tool, path: r.directory, exists: r.exists, files: r.files })),
        backfill: local.status.backfill ?? null,
      } : { enabled: false },
    };
  }
  ...
  function consolePayload() {
    ...
    const value = buildConsole({ store, registry, names, now, hub: hubInfo(), alerts: consoleAlerts(now), signals: signals(now), alertDay: alertDay?.read(now) ?? null });
