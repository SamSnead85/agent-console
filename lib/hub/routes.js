...
/**
 * What join attempts are counted against: an IPv4 address as it is, an IPv6
 * address by its /64, since one machine can hold any number of addresses in
 * its own /64.
 */
...
export function createReportingHandler({ config, registry, store, version, publicDir, onChange = () => {}, onEvent = () => {} }) {
  ...
  return async function handle(req, res, { secure }) {
    ...
    if (url === "/api/join") {
      // An address over its own limit is not counted in the total.
      const wait = joinPerAddress.take(joinAddressKey(ip)) || joinTotal.take("*");
      if (wait) {
