...
export function createConsoleHandler({ config, registry, store, names, local, admin, version, publicDir, reporting, git = null, interop = null }) {
  let cached = null;
  ...
  return {
    ...
    async handle(req, res) {
      ...
      if (config.interop && interopPath) {
        ...
        if (!authorization.startsWith('Bearer ') || !admin.secretMatches(authorization.slice(7))) {
          sendText(res, 403, 'console key required'); return;
        }
      }
