...
export function startDemo({ registry, store, fleet = null, activity = null, now = () => Date.now(), tickMs = 1000 }) {
  ...
  // The workstation's infra lane is the demo's failing shell. Its random
  // failures can all fall outside the five minutes shown, depending on the
  // second the demo starts; one failed shell call a whole minute ago is always
  // inside them, so the screen shows it every time.
  const failing = bookOf(spikeLane);
  if (failing) {
    const at = Math.floor(start / MINUTE) * MINUTE - MINUTE;
    failing.note(spikeLane.sessionHash, at, { kind: "shell" });
    failing.note(spikeLane.sessionHash, at, { error: 1 });
  }
  ...
  /* The demo's day: its listed alerts dated today, and one earlier alert an
     hour that the demo's short list no longer keeps, so the count and the
     kept list differ as they do on a busy day. */
  function alertDay(t = now()) {
    const d = new Date(t);
    const from = new Date(d.getFullYear(), d.getMonth(), d.getDate()).getTime();
    const today = alerts(t).filter((a) => a.at >= from);
    const earlier = Math.floor((t - from) / (60 * MINUTE));
    const byKind = { loop: 0, spike: 0, stall: 0 };
    for (const a of today) byKind[a.kind] += 1;
    byKind.loop += earlier;
    const pad = (n) => String(n).padStart(2, "0");
    return { date: `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}`, tz: Intl.DateTimeFormat().resolvedOptions().timeZone || "UTC",
      from, since: from, count: today.length + earlier, byKind, lastHour: today.filter((a) => !a.historical && a.at >= t - 60 * MINUTE).length, exact: true };
  }

  return { names, alerts, alertDay, stop: () => clearInterval(timer) };
}
