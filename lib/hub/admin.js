 ...
 *
 * scrape    with --interop, /metrics and the telemetry ingest take a bearer
 *           token: HMAC-SHA256 under the key of "agent-console/metrics/v1".
 *           `metrics-token` prints it. It is not the key and does not reveal
 *           it, the key itself is refused there, and a new key makes a new
 *           token, so the old one stops working.
 *
...
/** The bearer token for /metrics and the telemetry ingest, derived from the console's key. */
export function scrapeToken(key) {
  return crypto.createHmac("sha256", key).update(SCRAPE_TOKEN_LABEL).digest("base64url");
}
