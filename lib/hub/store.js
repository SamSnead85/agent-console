...
export function createStore({ dir = null, retentionMs, prices, now = () => Date.now(), rollupDays = ROLLUP_DAYS }) {
   ...
   */
  function growthOf(record) {
    const held = ids.get(record.id);
    ...
    for (const key of RUNNING) {
      ...
      const before = held.max[key];
      grown[key] = value === null ? 0 : before === null ? value : Math.max(0, value - before);
      if (value !== null && (before === null || value > before)) held.max[key] = value;
    }
  ...
  function index(record, deviceId, growth = false) {
    ...
    if (!growth) {
      ids.set(record.id, record.cumulative === true
        ? { minute, max: Object.fromEntries(RUNNING.map((key) => [key, Number.isSafeInteger(record[key]) ? record[key] : null])) }
        : minute);
    }
