...
export function createStore({ dir = null, retentionMs, prices, now = () => Date.now() }) {
  ...
  function load() {
    ...
    for (const name of safeList()) {
      ...
      if (recoverFile(file)) total.damaged += 1;
  ...
  function append(lines, day = dayOf(now())) {
    ...
    const handle = fs.openSync(path.join(dir, `records-${day}.ndjson`), "a", 0o600);
    let originalSize;
    try {
      originalSize = fs.fstatSync(handle).size;
      for (let i = 0; i < lines.length; i += 5000) {
        const chunk = Buffer.from(lines.slice(i, i + 5000).join("\n") + "\n");
        let offset = 0;
        while (offset < chunk.length) {
          const written = fs.writeSync(handle, chunk, offset, chunk.length - offset);
  ...
  function ingest(deviceId, records) {
    ...
    for (const record of records) {
      if (ids.has(record.id) || pendingIds.has(record.id)) { receipt.duplicate += 1; continue; }
      if (record.at < edge || record.at > future) { receipt.expired += 1; continue; }
      if (ids.size + pending.length >= MAX_INDEXED_RECORDS) { receipt.rejected.push({ id: record.id, because: "hub_full" }); continue; }
      pending.push(record);
    ...
    for (const record of pending) index(record, deviceId);
