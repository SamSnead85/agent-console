...
import { firstDay, BACKFILL_DAYS } from "./store.js";
 ...
 */
export function startLocalCollection({ registry, store, names, stateDir, roots = defaultRoots(), intervalMs = 5000, label, person, onTranscriptLine = null, journal = null, onError = () => {} }) {
  ...
  let stopped = false, timer = null, running = null, lastError = null, backfillSettled = false;
  ...
  const status = { device, lastRunAt: null, lastDurationMs: null, firstRunComplete: false, error: null, roots: roots.map((r) => r.tool), progress: null,
    // Every root looked at, and what was found there: the screen names them when nothing is found.
    rootsRead: roots.map((r) => ({ tool: r.tool, directory: r.directory, optional: Boolean(r.optional), exists: null, files: null })),
    backfill: null };
  // Walks everything once a minute and, between walks, only what can be changing (lib/collector/scanner.js).
  ...
  async function tick() {
    ...
    try {
      const result = await runOnce({
        ...
        // The first read goes back the console's 30 days; older days fill the
        // daily totals once. Asked until a pass completes without one pending.
        ...(backfillSettled ? {} : { backfillFromMs: Date.parse(firstDay(started) + "T00:00:00Z") }),
        scanner,
        onLocalLabel,
        onTranscriptLine,
        // Tool activity counts once the cursor that read it is written (lib/collector/activity.js).
        journal,
        ...
        sinkName: "hub",
        deliver: async (reportingDevice, records, freshness, meta = {}) => {
          const receipt = store.ingest(reportingDevice.id, records, meta.backfill ? { backfill: meta.backfill.from } : {});
          registry.touch(reportingDevice.id, { freshness });
          return receipt;
        },
      });
      // Delivered whole: the first read's past days are done.
      if (result?.backfill) { store.endBackfill(device.id); status.backfill = { from: result.backfill.from, days: BACKFILL_DAYS, complete: true }; }
      backfillSettled = true;
      if (Array.isArray(result?.coverage?.roots)) status.rootsRead = result.coverage.roots.map((r, i) => ({ ...r, optional: Boolean(roots[i]?.optional) }));
      // What this machine's transcripts held that could not be counted, shown
      // beside the figures instead of being thrown away (docs/accounting.md §3.2).
      if (result?.coverage?.coverageDebt) registry.touch(device.id, { coverage: result.coverage.coverageDebt });
      names.save();
      status.error = null;
      lastError = null;
    } catch (error) {
