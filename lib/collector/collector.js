 ...
 */
export async function collect({ directory, roots = defaultRoots(), sinceMs = null, onLocalLabel = null, onProgress = null, onTranscriptLine = null, journal = null, shareLabels = true, projectKey = null, maxLineBytes = MAX_LINE }) {
  ...
  try {
      ...
      try {
        for (const filename of files) {
          ...
          try {
            ...
            journal?.file?.(true);
            coverage.filesRead++;
          } catch { journal?.file?.(false); coverage.unreadableFiles++; }
          if (onProgress) { progress.records = added; onProgress({ ...progress }); }
        }
      } catch { journal?.file?.(false); coverage.unreadableFiles++; }
    }
    ...
    if (dirty || added || journal?.changed) await atomicJSON(cursorFile, cursor);
    journal?.commit?.();
  } catch (error) {
    journal?.abort?.();
    throw error;
...
export async function runOnce(options = {}) {
  ...
  return withLock(directory, async () => {
    ...
    const { cursor, hashIdentity, device, coverage, added, changed } = await collect({ directory, roots: options.roots,
      ...
      onTranscriptLine: options.onTranscriptLine ?? null, journal: options.journal ?? null, onProgress,
    ...
    if (changed || emitted || offset !== start || compact || refresh || extrasAcked) await atomicJSON(path.join(directory, 'cursor-v2.json'), cursor);
