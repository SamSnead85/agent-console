 ...
 * is archived. Every one of these that exists is read; a root that is optional
 * and absent is not missing coverage. `claudeRoot` and `codexRoot` (the
 * --claude-root and --codex-root options) replace the list for their tool.
 ...
 */
export function transcriptRoots({ home = os.homedir(), env = process.env, claudeRoot = null, codexRoot = null } = {}) {
  const roots = [];
      ...
      try {
        ...
        if (root.tool === 'codex' && !head) {
          ...
          if (from !== undefined && from !== fileKey && cursor.sources[from]) {
            ...
            if (!present.has(from)) {
              known = cursor.sources[fileKey] = cursor.sources[from];
              delete cursor.sources[from];
...
export async function runOnce(options = {}) {
  ...
  try { return await withLock(directory, async () => {
    ...
    // A first read (`backfillFromMs`: the console's 30 UTC days): this machine
    // has never delivered, or its first delivery is still under way. It reads
    // back that far, and every delivery says so until it is complete, so the
    // console can fill whole past days once (lib/hub/store.js).
    let backfill = null;
    if (Number.isFinite(options.backfillFromMs) && (options.post || options.deliver)) {
      const saved = await heldCursor(options.scanner, path.join(directory, 'cursor-v2.json')) ?? await readJSON(path.join(directory, 'cursor-v2.json'), null);
      const sinks = saved && saved.sinks && typeof saved.sinks === 'object' ? Object.values(saved.sinks) : [];
      const pending = sinks.find((s) => s && s.backfill && typeof s.backfill.from === 'string');
      if (pending) backfill = { from: pending.backfill.from };
      else if (!sinks.length) backfill = { from: new Date(Math.floor(options.backfillFromMs / 86_400_000) * 86_400_000).toISOString().slice(0, 10) };
    }
    const sinceMs = backfill ? Math.min(options.sinceMs ?? Infinity, Date.parse(backfill.from + 'T00:00:00Z')) : options.sinceMs ?? null;
    const { cursor, hashIdentity, device, coverage, added, changed } = await collect({ directory, roots: options.roots,
      ...
      onTranscriptLine: options.onTranscriptLine ?? null, journal: options.journal ?? null, scanner: backfill ? null : options.scanner ?? null, onProgress,
    ...
    if (options.deliver) {
      ...
      const flush = async () => {
        const part = await options.deliver(device, batch, freshness, { backfill });
    ...
    if (options.deliver) {
      ...
      try {
        ...
        // A first delivery is remembered before it is attempted, so a retry after a failure still says so.
        if (backfill && !cursor.sinks[sink]) { cursor.sinks[sink] = { offset: start, lastSyncedAt: null, backfill }; await save(true); }
        receipt = await postRecords(options.post, device, records, {
          ...
          onBatch: async (batch, { delivered, total }) => {
            ...
            cursor.sinks[sink] = { offset: done ? offset : ends[delivered - 1], lastSyncedAt: cursor.lastSyncedAt,
              ...(done ? {} : { catchUp: { delivered: base + delivered, total: backlog.total }, ...(backfill ? { backfill } : {}) }) };
    ...
    return { added, emitted, coverage, receipt, freshness, backlog: { delivered: backlog.total, total: backlog.total }, measurement: syncMeasurement(now), backfill };
