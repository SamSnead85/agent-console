...
// Exactly the receiver's engagement rule. A label this collector accepted but
// the receiver did not would refuse the WHOLE batch, so an unusable label is
// dropped to null here instead of poisoning every record beside it.
const ENGAGEMENT = /^(?:[a-z][a-z0-9-]{1,47}|[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})$/;
...
async function repairSpool(filename) {
  ...
  try {
    ...
    while (end > 0) {
      ...
      if (newline >= 0) { await handle.truncate(start + newline + 1); return; }
      end = start;
    }
 ...
 */
export async function collect({ directory, roots = defaultRoots(), sinceMs = null, onLocalLabel = null }) {
  const { hashIdentity, recordId, device, fingerprint } = await initialize(directory);
  ...
  const labels = await readJSON(labelsFile, {});
...
export async function runOnce(options = {}) {
  ...
  return withLock(directory, async () => {
    ...
    if (options.deliver) {
      ...
      receipt = await postRecords(options.post, device, records, { freshness, token: options.token, allowHttp: options.allowHttp, ...(options.transport ?? {}) });
    ...
    return { added, emitted: records.length, coverage, receipt, freshness, measurement: syncMeasurement(now) };
  });
}
