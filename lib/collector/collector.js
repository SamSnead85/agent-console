...
export function pruneParserState(parser, now = Date.now()) {
  ...
  const maps = ['claudeUsage', 'codexEventIds', 'claudeFallbackSent'].filter(key => parser[key] && typeof parser[key] === 'object');
 ...
 */
export async function collect({ directory, roots = defaultRoots(), sinceMs = null, onLocalLabel = null, onProgress = null, onTranscriptLine = null, shareLabels = true, projectKey = null }) {
  ...
  return withLock(directory, async () => {
    ...
    if (options.deliver) {
      ...
      const flush = async () => {
        ...
        emitted += batch.length; batch = []; sent = true;
      };
      for await (const item of lines(spoolPath, start)) {
        const record = fromSpool(item.line);
        if (!seen.has(record.id)) { batch.push(record); seen.add(record.id); }
        offset = item.endOffset;
