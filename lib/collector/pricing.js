...
function rowFor(priceTable, model) {
  ...
  let found;
  let matches = priceTable.rows.filter((row) => row?.model === model);
  // An alias the vendor publishes for a dated id (`claude-haiku-4-5` for
  // `claude-haiku-4-5-20251001`) prices at its target's row, never a guess:
  // each alias names its target, its source page and the day it was checked.
  if (matches.length === 0) {
    const alias = aliasOf(priceTable, model);
    if (alias) matches = priceTable.rows.filter((row) => row?.model === alias);
  }
  const row = matches[0];
...
/** The dated model id a published alias stands for, or null. */
export function aliasOf(priceTable, model) {
  if (!Array.isArray(priceTable?.aliases)) return null;
  const entry = priceTable.aliases.find((a) => a?.model === model);
  if (!entry || typeof entry.aliasOf !== "string" || typeof entry.source !== "string" || !entry.source.startsWith("https://")
    || !/^\d{4}-\d{2}-\d{2}$/u.test(entry.verifiedOn ?? "")) return null;
