...
function claude(line, context, state) {
  ...
  // A line uuid already seen (a re-written line, or a copy in another file).
  // Usage that grew is a new increment with an identity of its own, so the
  // hub's first-writer-wins cannot drop it. The same transcript rewriting a
  // line with LOWER usage cannot un-count what was reported, and is surfaced
  // as coverage debt (§3.1); a lower copy in another file is only a snapshot.
  ...
  if (seenLine !== null && lineTotal < seenLine && seenIn === tag) debt(state, 'revisedDown');
...
function codex(line, context, state) {
  ...
  if (records.length) { state.codexEventIds[identityKey] = records[0].at; state.cumulativeCounted = true; }
  return { records, state };
}
...
function codexResponse(line, payload, context, state) {
  ...
  // Records are written before the running total in every rollout observed.
  // One that arrives after running-total events were already counted for this
  // thread would count the same responses twice, so it is not counted.
  if (state.cumulativeCounted && !state.usageRecords) { debt(state, 'lateUsageRecord'); return { records: [], state }; }
