...
function record(tool, context, state, timestamp, usage, messageId, observedChange = false) {
  const at = minute(timestamp);
...
function claude(line, context, state) {
  ...
  if (!uuid) {
    ...
    const records = record('claude-code', context, state, line.timestamp, highWater, recordMessage);
    if (records.length) state.claudeFallbackSent[fallbackKey] = true;
    return { records, state };
  }
  return { records: record('claude-code', context, state, line.timestamp, delta, recordMessage), state };
}
...
function codex(line, context, state) {
  ...
  for (const key of CODEX_CLASSES) {
    delta[key] = cumulative[key] === null ? null : !previous ? cumulative[key]
      : count(previous[key]) === null ? null : cumulative[key] - previous[key];
  }
...
export function parseLine(tool, line, context, state = {}) {
  ...
  return canonical === 'claude-code' ? claude(value, context, next) : codex(value, context, next);
}
