...
export function extrasFor(envelope) {
  ...
  if (share && ((share.alerts === "off" && alerts !== undefined) || (share.activity === "off" && activity !== undefined))) {
    throw fail("invalid_share", "An envelope carried a list its sharing declaration says is off.");
  }
  return { share, alerts, activity };
}
...
export async function postRecords(url, device, records, options = {}) {
  ...
  const { share, alerts, activity } = extrasFor({ share: options.share, alerts: unlessEmpty(options.alerts), activity: unlessEmpty(options.activity) });
