 ...
 * not send it, and a console that does not know it ignores it. Present (even
 * empty) means "this machine watches for alerts".
 */
...
/**
 * Tool activity, sent only with --share-tool-activity: per session and
 * minute, calls by kind (eight fixed kinds, all present), results as ok and
 * error, and the kind and minute of the session's last tool. No tool name,
 * argument, path or server name has a place to go.
 */
export function activityFor(value) {
  ...
  for (const e of value) {
    if (!exactKeys(e, ["sessionHash", "at", "calls", "results", "lastTool"]) || !HASH.test(e.sessionHash) || !minuteAt(e.at)
  ...
  return value.map((e) => ({ sessionHash: e.sessionHash, at: e.at, calls: { ...e.calls }, results: { ...e.results },
...
export async function postRecords(url, device, records, options = {}) {
  ...
  // Opt-in extras ride on the first envelope only (never repeated per batch).
  const alerts = alertsFor(options.alerts);
  const activity = activityFor(options.activity);
  ...
  for (let start = 0; start < deliveryLength; start += 500) {
    ...
    const envelope = { v: 1, device, freshness, records: rows, ...(coverage ? { coverage } : {}),
      ...(start === 0 && alerts ? { alerts } : {}), ...(start === 0 && activity ? { activity } : {}) };
