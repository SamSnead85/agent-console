...
const RECORD_KEYS = [...LEGACY_RECORD_KEYS, "continuation"];
...
export function validateRecords(records, device) {
  ...
  for (const row of records) {
    ...
    if (!(current || exactKeys(row, LEGACY_RECORD_KEYS)) || (current && typeof row.continuation !== "boolean")
      || !text(row.id) || ids.has(row.id)
...
export async function postRecords(url, device, records, options = {}) {
  ...
  for (let start = 0; start < deliveryLength; start += 500) {
    ...
    const envelope = { v: 1, device, freshness, records: rows };
