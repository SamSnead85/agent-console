 ...
 * [{ words, inputs, strings, piped, scope, cut, substCommand }]: `inputs` are
 * `<` redirection sources and `strings` are `<<<` here-strings; output
 * redirection targets are dropped. `piped` marks a command whose standard
...
export function shellSegments(command) {
  ...
  const endSegment = (cut = false) => {
    endWord();
    redirect = null;
    if (words.length || inputs.length || strings.length) out.push({ words, inputs, strings, piped, scope: scope(), cut, substCommand });
    words = []; inputs = []; strings = [];
...
function classifyCommand(command, cwd, repoRoot, home, found, depth) {
  ...
  const dirOf = (scope) => {
    ...
    const chain = [];
    for (let at = scope; at !== null && at !== undefined && !dirs.has(at); at = segments.parents.get(at)) chain.push(at);
    for (const at of chain.reverse()) {
      const parent = segments.parents.get(at);
  ...
  for (const segment of segments) {
    const { words, strings, piped, scope, cut, substCommand } = segment;
    ...
    if (words.some((word) => /^(?:rm|remove-item)$/u.test(base(word))) && deleteOutside(words, dir, repoRoot, home, root)) found.add('delete_outside_repo');
    if (depth < 4) {
      for (const inner of nested(words)) classifyCommand(inner, dir, repoRoot, home, found, depth + 1);
      if (SHELLS.has(name) || POWERSHELLS.has(name)) for (const text of strings) classifyCommand(text, dir, repoRoot, home, found, depth + 1);
    }
    if (['cd', 'chdir', 'pushd', 'popd', 'set-location', 'sl', 'push-location', 'pop-location'].includes(name)) {
      ...
      dirs.set(scope, changeDirectory([kind, ...run.slice(1)], dir, home, stacks.get(scope), cut, repoRoot));
    }
  }
}
