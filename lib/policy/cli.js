...
export function policyApply(root, { orgFile = null, stateDir = null } = {}) {
  ...
  try {
    ...
    writePrivate(manifestFile, json({ version: 1, root, files: entries, createdDirs }));
  } catch (error) {
...
export const policyUsage = (cmd) => `Usage: ${cmd} policy diff|apply|remove [--project <path>] [--org-policy <file>]
...
export function mainPolicy(args, { cmd = 'node bin/agent-console.mjs', out = process.stdout } = {}) {
  ...
  if (!['diff', 'apply', 'remove'].includes(command)) throw new Error(`"${command}" is not a policy command.\n${policyUsage(cmd)}`);
