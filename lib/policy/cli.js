...
const real = (filename) => { try { return fs.realpathSync.native(filename); } catch { return null; } };
 ...
 */
const statOf = (filename) => { try { return fs.statSync(filename); } catch { return null; } };
const sameFile = (a, b) => Boolean(a && b && a.dev === b.dev && a.ino === b.ino);

/**
 * True when <root>/.claude is the user-level Claude directory. Compared by
 * file identity, not by spelling: a miscased or firmlinked path names the
 * same directory on macOS.
 */
function userLevel(root, realRoot) {
  const userDirs = [path.join(os.homedir(), '.claude'), process.env.CLAUDE_CONFIG_DIR].filter(Boolean).map((dir) => path.resolve(dir));
  const spelled = userDirs.flatMap((dir) => [real(dir), real(path.dirname(dir)) && path.join(real(path.dirname(dir)), path.basename(dir))]);
  if (spelled.includes(path.join(realRoot, '.claude'))) return true;
  const candidate = statOf(path.join(root, '.claude'));
  const rootStat = statOf(root);
  return userDirs.some((dir) => candidate && sameFile(candidate, statOf(dir))
    || path.basename(dir).toLowerCase() === '.claude' && sameFile(rootStat, statOf(path.dirname(dir))));
}

function locate(root, filename, create = false) {
  const realRoot = fs.realpathSync.native(root);
  if (userLevel(root, realRoot)) throw new Error('Refusing to write user-level Claude settings');
...
export function policyApply(root, { orgFile = null, stateDir = null } = {}) {
  ...
  // Directories this apply creates, deepest first, so remove can take them away again.
  const createdDirs = [...new Set([...files.keys()].flatMap((filename) => {
    const parts = path.relative(root, path.dirname(filename)).split(path.sep);
    return parts.map((_, index) => path.join(root, ...parts.slice(0, index + 1)));
  }))].filter((candidate) => !lstat(candidate)).sort((a, b) => b.length - a.length);
  ...
  try {
    ...
    writePrivate(manifestFile, json({ version: 1, root, files: entries, createdDirs, policyDigest: hash(json(policy)), orgPolicy: orgFile ? path.resolve(orgFile) : null }));
  } catch (error) {
...
export function policyRemove(root, { stateDir = null } = {}) {
  ...
  const createdDirs = Array.isArray(manifest.createdDirs) ? manifest.createdDirs : [];
  const claudeDir = path.join(root, '.claude');
  if (createdDirs.some((dir) => typeof dir !== 'string' || dir !== claudeDir && !dir.startsWith(claudeDir + path.sep)))
    throw new Error('Invalid policy install manifest');
  const restored = [];
  const deleted = [];
  for (const entry of manifest.files) {
    if (entry.backup) {
      write(root, entry.path, fs.readFileSync(path.join(path.dirname(manifestFile), entry.backup)), entry.mode);
      restored.push(entry.path);
    } else {
      unlink(root, entry.path);
      deleted.push(entry.path);
    }
  }
  // Take away the directories apply made, if nothing else was put in them since.
  for (const dir of [...createdDirs].sort((a, b) => b.length - a.length)) {
    ...
    try { fs.rmdirSync(dir); } catch { /* not empty: something else lives there now */ }
  }
  fs.rmSync(path.dirname(manifestFile), { recursive: true });
  return { removed: true, changes: manifest.files.map((entry) => entry.path), restored, deleted };
}
...
export const policyUsage = (cmd) => `Usage: ${cmd} policy diff|apply|remove|status [--project <path>] [--org-policy <file>] [--json]

  diff     show what apply would create or change; writes nothing
  apply    install the repository's agent-policy.yaml as project Claude Code files, with private backups
  remove   restore the settings from before apply and delete the generated files
  status   check installed file integrity; --json prints structured status (runtime enforcement remains unverified)

Run from the repository root, or name it with --project. See docs/policy.md.
`;

/** Returns the exit code. `cmd` is how this copy is run (lib/invocation.js). */
export function mainPolicy(args, { cmd = 'node bin/agent-console.mjs', out = process.stdout } = {}) {
  const command = args[0];
  if (command === undefined || ['help', '--help', '-h'].includes(command) || args.includes('--help') || args.includes('-h')) {
    out.write(policyUsage(cmd));
    return command === undefined ? 2 : 0;
  }
  if (!['diff', 'apply', 'remove', 'status'].includes(command)) throw new Error(`"${command}" is not a policy command.\n${policyUsage(cmd)}`);
  ...
  if (command === 'status') {
    ...
    if (args.includes('--json')) out.write(json(status));
    else {
      out.write(`Policy: ${status.state}\nRuntime enforcement: unverified\n`);
      for (const issue of status.issues) out.write(`${issue.reason}: ${issue.file}\n`);
      out.write('Hard budgets: not enforced. Gateway: not configured.\n');
    }
    return status.state === 'installed' ? 0 : 1;
  }
  const note = 'Not enforceable yet: hard token/dollar budgets, verifying-test proof, failed-attempt count.\n';
  if (command === 'diff') {
    const result = policyDiff(root, orgFile);
    for (const change of result.changes) out.write(`${change.action}: ${change.path}\n`);
    out.write(note);
  } else if (command === 'apply') {
    const result = policyApply(root, { orgFile });
    for (const change of result.changes) out.write(`${change.action}: ${change.path}\n`);
    out.write(result.alreadyApplied ? 'Policy already applied.\n' : `Private backups: ${result.backup}\n`);
    out.write(note);
  } else {
    const result = policyRemove(root);
    for (const filename of result.restored || []) out.write(`restored: ${filename}\n`);
    for (const filename of result.deleted || []) out.write(`removed: ${filename}\n`);
    if (!result.removed) out.write('No applied policy found.\n');
  }
  return 0;
}
