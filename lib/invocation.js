...
export function joinCommand(version, link) {
  ...
  return `npx --yes ${releaseUrl(version)} join '${link}'`;
}

/** How to run this copy again, quoted so a path with spaces survives a shell. */
export function invocation(version, entry = process.argv[1], executable = isSea() ? process.execPath : null) {
  if (executable) return executableCommand(executable);
...
export function executableCommand(executable, { env = process.env, platform = process.platform, exists = fs.existsSync, realpath = fs.realpathSync } = {}) {
  ...
  const same = (a, b) => { try { a = realpath(a); b = realpath(b); } catch { /* compare as given */ } return platform === "win32" ? a.toLowerCase() === b.toLowerCase() : a === b; };
  for (const dir of String(env.PATH ?? env.Path ?? "").split(p.delimiter)) {
    if (!dir || !exists(p.join(dir, file))) continue;
    if (same(p.join(dir, file), executable)) return file.replace(/\.exe$/iu, "");
    break; // another copy comes first on PATH
  }
  if (/^[\w./:\\-]+$/u.test(executable)) return executable;
