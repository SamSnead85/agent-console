      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          if (entry.name === "memory" || entry.name === "tool-results")
            continue;
          stack.push(full);
        } else if (entry.name.endsWith(".jsonl") && entry.name !== "agent-journal.jsonl" && entry.name !== "journal.jsonl") {
  ...
  for (const state of store.files.values()) {
    ...
    if (state.isSub) {
      ...
      s.agents.push({
        ...
        cost: ownUnpriced ? null : ownCost,
        pricedCost: ownCost,
        unpriced: ownUnpriced,
        hot,
        live,
        lastTs: state.lastTs,
        mtime: state.mtime,
