...
async function defaultMergesForPeriod(store, root, fromMs) {
  const key = root + '|' + Math.floor(fromMs / 60_000);
  ...
  store.defaultMerges.set(key, { at: Date.now(), value });
  return value;
}
 ...
 */
export async function gitStatsForPeriod(store, dirs, fromMs) {
  const roots = new Set();
  ...
  for (const root of roots) {
    ...
    repos.push({
      ...
      defaultMerges: fromMs === null ? null : await defaultMergesForPeriod(store, root, fromMs),
    });
