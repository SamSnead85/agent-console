...
export function createGitStatsStore() {
  return { cache: new Map(), toplevel: new Map(), defaultMerges: new Map() };
}

/** Count integration commits on a locally known default branch; never fetch. */
async function defaultMergesForPeriod(store, root, fromMs, periodKey) {
  const now = Date.now();
  for (const [oldKey, entry] of store.defaultMerges) {
    if (now - entry.at >= CACHE_MS) store.defaultMerges.delete(oldKey);
  }
  const key = root + '|' + (periodKey || Math.round((now - fromMs) / 3_600_000) + 'h');
  ...
  if (!ref) return null;
  const result = await git(root, ['log', ref, '--first-parent', '--since=' + new Date(fromMs).toISOString(),
 ...
 */
async function statsForRoot(store, root, fromMs) {
  const cacheKey =
    root + "|" + (fromMs === null ? "all" : Math.floor(fromMs / 60_000));
 ...
 */
export async function gitStatsForPeriod(store, dirs, fromMs, { periodKey = null } = {}) {
  const roots = new Set();
  ...
  for (const root of roots) {
    const stats = await statsForRoot(store, root, fromMs);
    if (stats.error) {
    ...
    repos.push({
      ...
      defaultMerges: fromMs === null ? null : await defaultMergesForPeriod(store, root, fromMs, periodKey),
    });
