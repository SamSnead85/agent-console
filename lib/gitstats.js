 ...
 * the fleet works in is on this disk.
 *
...
export function createGitStatsStore() {
  return { cache: new Map(), toplevel: new Map() };
}
...
export function parseNumstat(stdout, fromMs) {
  ...
  for (const block of stdout.split(REC)) {
    ...
    // --since already filters, but the guard keeps the maths honest if a
...
export async function gitStatsForPeriod(store, dirs, fromMs) {
  ...
  // "<root>#<n>" here exactly as the fleet total is. Counting bare numbers
