...
export function buildGlossary(input) {
  ...
  const entries = [
    ...
    },
    {
      ...
      kind: "measured",
      short:
        "Context the model re-read from its prompt cache, billed at " +
        CACHE_READ_MULT +
        "× the input rate. Most of the tokens, a small share of the dollars.",
      body:
        "Every turn of a long conversation re-sends the whole conversation. Prompt caching means the vendor keeps that prefix warm and charges " +
        CACHE_READ_MULT * 100 +
        "% of the input rate to read it back instead of the full rate to process it again. That is why cache read is usually the largest token class on this screen and one of the smallest cost classes — and why a large cache-read figure is the cache WORKING, not a leak. The cheapest token here is a cache read; the way to reduce it is fewer or shorter sessions, not a different setting.",
    },
    {
      id: "cache-write",
      term: "cache write",
