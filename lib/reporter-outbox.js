  ...
  function bound(s, t) {
    for (const [kind, , age, max] of KINDS) {
      ...
      if (dropped.length) {
        ...
        // Keep the actual interval: accepted observations can be ahead of this
        // clock within the skew allowance. Clamping only its end reverses it.
        recordLoss(s, kind, dropped.length, Math.min(...times), Math.max(...times));
      }
    }
    return s;
  }
