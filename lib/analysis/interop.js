...
/* What each source's `total` adds up: OTel's Claude Code metric counts cache
   reads and writes as their own types, the gateways' totals do not. */
export const TOKEN_DEFINITION = { otel: 'input+output+cache', kong: 'input+output', litellm: 'input+output' };

/**
 * `stats` ({ dedupedSamples, cumulativeIgnored }) is the OTel adapter's own
 * count of re-sent points dropped and cumulative points not summed.
 */
export function summarizeInterop(samples, now = Date.now(), stats = null) {
  const result = Object.fromEntries(SOURCES.map((source) => [source, { available: false,
    tokens: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }, receivedAt: null, tokenDefinition: TOKEN_DEFINITION[source] }]));
  result.otel.dedupedSamples = safeCount(stats?.dedupedSamples) ?? 0;
  result.otel.cumulativeIgnored = safeCount(stats?.cumulativeIgnored) ?? 0;
